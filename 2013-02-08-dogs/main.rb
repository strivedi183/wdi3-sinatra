...
post '/dogs/:dog_id' do
  sql = "update dogs set name = '#{params["name"]}', image = '#{params["photo"]}', breed = '#{params["breed"]}' where id = #{params['dog_id']}"
  run_sql(sql)
  redirect to("/dogs")
end
...
post '/create' do
  sql = "insert into dogs (name, image, breed) values ('#{params['name']}', '#{params['photo']}', '#{params['breed']}')"
  run_sql(sql)
  redirect to("/dogs")
end

def run_sql(sql)
  conn = PG.connect(:dbname=>'dogs_app', :host => 'localhost')
